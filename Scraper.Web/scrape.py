...
import json
import yaml
from Core.Websites import functions

# Systems check
with open("config.yml", "r") as config:
    ymlData = yaml.load(config, Loader=yaml.FullLoader)

with open("Core/options.yml", "r") as options:
    optionsData = yaml.load(options, Loader=yaml.FullLoader)

userInquiry = "3060"

# Variables
activeWebsites = ymlData['active-websites']
searchOptions = ymlData['search-options']
htmlVars = optionsData
...
for websites in activeWebsites:
    
    if not activeWebsites[websites]:
        continue

    content = functions.startConnection(userInquiry, urlSession, htmlVars[websites]['search-url'], "lxml")
    maxNumberPages = functions.numberOfPages(content, htmlVars[websites]['pagnation-class'])
    
    

#searchPage = urlSession.get(url, HEADERS).text
